...
describe('Section 1: Functional tests', () => {
    ...
    it('User can use only same both first and validation passwords', ()=>{
        ...
        cy.get('#username').type('RiinaV')
        cy.get('#email').type('riina@email.ee')
        cy.get('[data-cy="name"]').type('Riina')
        cy.get('#lastName').type('Veldi')
    ...
    it('User can not submit form when username field is empty', () => {
...
describe('Section 2: Visual tests', () => {
    ...
    it('Check that the second picture is correct and has correct size', () => {
        ...
        // get element and check its parameter height, to less than 178 and greater than 100
        cy.get('[data-cy="cypress_logo"]').invoke('height').should('be.lessThan', 178).and('be.greaterThan', 80)
    });
    ...
    it('Check navigation part 2', () => {
        ...
        cy.get('nav').children().eq(1).should('be.visible').and('have.attr', 'href', 'registration_form_3.html')
        .click()
    ...
    it.only('Animal dropdown is correct', () => {
        ...
        //Verify values in the animal dropdown
...
function inputValidData(username) {
    ...
    cy.get('input[data-testid="user"]').type(RiinaV)
    cy.get('#email').type('riina@email.ee')
    cy.get('[data-cy="name"]').type('Riina')
    cy.get('#lastName').type('Veldi')
    cy.get('[data-testid="phoneNumberTestId"]').type('55667788')
    // If element has multiple classes, then one of them can be used
    cy.get('#password').type('Riina123')
    cy.get('#confirm').type('Riina123')
    cy.get('h2').contains('Password').click()
}